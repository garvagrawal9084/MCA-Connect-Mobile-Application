...
## Important 
 ...
 - Maintain the `learning/` directory: Create a dedicated, thorough educational markdown file for each library, architectural pattern, and core topic in this project (e.g. Expo Router, Zustand, NativeWind, SecureStore, API Client, Auth Refresh Bootstrap, Reanimated, Logging). When introducing a new library or complex technique, add a new file to `learning/` explaining what it is, why we use it, how it is implemented in this codebase, and practical code examples.
